...
#import backup as student

import dense as student
...
def test_network(net,testloader,print_confusion=False):
    ...
    str=', {0} test {1:.2f}% \n'.format(total_images,model_accuracy)
    f = open("log_save/dense.txt", "a")
    f.write(str)
    f.close()
...
def main():
    ...
    if student.train_val_split == 1:
        ...
        trainloader = torch.utils.data.DataLoader(data,
                            batch_size=student.batch_size, shuffle=True, num_workers=1);
    else:
        ...
        testloader = torch.utils.data.DataLoader(testset, 
                            batch_size=student.batch_size, shuffle=False, num_workers=1)
    ...
    for epoch in range(1,student.epochs+1):
        ...
        tmp_str = 'ep {0}, loss: {1:.2f}, {2} train {3:.2f}%'.format(epoch, total_loss, total_images, model_accuracy)
        f = open("log_save/dense1.txt", "a")
        f.write(tmp_str)
        f.close()
        ...
        if epoch % 10 == 0:
            ...
            filename = 'model_saves/dense1/dense_'+ str(epoch)+'.pth'
